use super::path::{get_value_at_path, parse_path, set_value_at_path};
                ...
                for (key, val) in map {
                    match key.as_str() {
                        "$set" => match val {
                            Value::Object(fields) => {
                                for (field, field_val) in fields {
                                    modifiers.push(UpdateModifier::Set(
                                        field.clone(),
                                        field_val.clone(),
                                    ));
                                }
                        ...
                        "$inc" => match val {
                            Value::Object(fields) => {
                                for (field, field_val) in fields {
                                    modifiers.push(UpdateModifier::Inc(
                                        field.clone(),
                                        field_val.clone(),
                                    ));
                                }
        ...
        match self {
                            ...
                            for segment in parent_path {
                                match current {
                                    Value::Object(parent_map) => {
                                        current = parent_map.get_mut(segment).ok_or_else(|| {
                                            Error::CorruptedDatabase {
                                    ...
                                    _ => {
                                        return Err(Error::CorruptedDatabase {
                                            reason: format!(
                                                "Cannot unset field in non-object at path {}",
                                                path_str
                                            ),
                                        });
                                    }
            ...
            UpdateModifier::Inc(path_str, increment) => {
                ...
                let new_value = match (current_value, increment) {
                    ...
                    (Some(Value::Int(current)), Value::Float(inc)) => {
                        Value::Float(current as f64 + inc)
                    }
                    (Some(Value::Float(current)), Value::Int(inc)) => {
                        Value::Float(current + *inc as f64)
                    }
                    ...
                    _ => {
                        return Err(Error::CorruptedDatabase {
                            reason: format!(
                                "Cannot increment non-numeric value at path {}",
                                path_str
                            ),
                        });
                    }
...
mod tests {
    ...
    fn test_update_modifier_set() {
        ...
        assert_eq!(
            get_value_at_path(&doc, &parse_path("age")),
            Some(Value::Int(31))
        );
    }
